...
# 4. baostock 获取历史PE/PB和基本面数据
print("[信息] 登录 baostock...")
bs.login()
...
try:
    ...
    for bc in bank_codes:
        try:
            b_info = dict(zip(ak.stock_individual_info_em(symbol=bc)['item'], ak.stock_individual_info_em(symbol=bc)['value']))
            b_hist = ak.stock_zh_a_hist(symbol=bc, period="daily", start_date=(datetime.now()-timedelta(days=30)).strftime("%Y%m%d"), end_date=end_date, adjust="qfq")
            if not b_hist.empty:
                b_close = b_hist['收盘'].astype(float)
                b_change_5 = (b_close.iloc[-1] / b_close.iloc[-6] - 1) * 100 if len(b_close) >= 6 else 0
                b_change_20 = (b_close.iloc[-1] / b_close.iloc[-21] - 1) * 100 if len(b_close) >= 21 else 0
                # 年初至今
                b_ytd = ak.stock_zh_a_hist(symbol=bc, period="daily", start_date="20250101", end_date=end_date, adjust="qfq")
                b_change_ytd = (b_ytd['收盘'].astype(float).iloc[-1] / b_ytd['收盘'].astype(float).iloc[0] - 1) * 100 if not b_ytd.empty else 0
            else:
                b_change_5 = b_change_20 = b_change_ytd = 0
            bank_compare_data.append({
                '代码': bc,
                '名称': b_info.get('股票简称', bc),
                '总市值': float(b_info.get('总市值', 0)) / 1e8,
                'PE': b_info.get('市盈率-动态', 'N/A'),
                '5日涨跌幅': f"{b_change_5:.2f}%",
                '20日涨跌幅': f"{b_change_20:.2f}%",
                '年初至今涨跌幅': f"{b_change_ytd:.2f}%"
            })
        except Exception as e:
            pass
except Exception as e:
    print(f"[警告] 银行对比获取失败: {e}")

bank_compare = pd.DataFrame(bank_compare_data)
industry_pe = None
if not bank_compare.empty and 'PE' in bank_compare.columns:
    try:
        industry_pe = bank_compare[bank_compare['PE'] != 'N/A']['PE'].astype(float).mean()
    except:
        pass
...
if not bank_compare.empty:
    ...
    for _, row in bank_compare.iterrows():
        lines.append(f"| {row['名称']} | {row['总市值']:.0f} | {row['PE']} | {row['5日涨跌幅']} | {row['20日涨跌幅']} | {row['年初至今涨跌幅']} |")
else:
